<?php
  ...
  $sql = "SELECT * FROM `property` WHERE (`vendor_id` = '$vendor_id') AND `status` != '-1'";
  ...
  if($num > 0){
    while($rows = mysqli_fetch_array($query,MYSQLI_ASSOC)){
      ...
      $result[count($result) - 1]['type'] = json_decode($rows['type'],true);
      $result[count($result) - 1]['amenities'] = json_decode($rows['amenities'],true);
      $result[count($result) - 1]['facilities'] = json_decode($rows['facilities'],true);
...
<html lang="en">
  ...
  <body class="az-body az-body-sidebar">
    ...
    <?php
      $options = array('page' => 'property','subpage'=>'list');
    ...
    <div class="az-content-header d-block d-md-flex">
        <div>
          <h2 class="az-content-title tx-24 mg-b-5 mg-b-lg-8">Manage properties</h2>
          <p class="mg-b-0">View, delete and modify the status of any property.</p>
        </div>
            ...
            <table id="eventList" class="table" style="width: 100%;">
              <thead>
                <tr role="row">
                  ...
                  <th rowspan="1" colspan="1" style="width: 15%;">Property ID</th>
                  <th rowspan="1" colspan="1" style="width: 25%;">Type</th>
                  <th rowspan="1" colspan="1" style="width: 20%;">Country</th>
...
EOT;
            for($i = 0; $i < count($result); $i++){
              ...
              $type = '';
              for($j = 0; $j < count($result[$i]['type']); $j++){
                $append = $j != count($result[$i]['type']) - 1 ? ', ' : '';
                $type .= $result[$i]['type'][$j].$append;
              }
              echo <<<EOT
              <tr>
                ...
                <td>{$result[$i]['property_id']}</td>
                <td>{$type}</td>
                <td>{$result[$i]['country']}</td>
                <td>{$result[$i]['price']}</td>
                <td style="display:flex;justify-content:space-around;align-items:flex-start">
                  <div class="az-toggle az-toggle-success switchstatus {$status}" data-id="{$result[$i]['property_id']}"><span></span></div>
                  <img src="../../img/cancel.svg" class="deleterow" data-toggle="modal" data-target="#deletemodal" style="cursor:pointer;" data-id="{$result[$i]['property_id']}" width="30px"/>
                </td>
            </tr>
EOT;
            }
        ...
        $(document).ready(function() {
            ...
            $('.switchstatus').click(function(e){
              ...
              $.ajax({
                url:'action.php',
                method:'post',
                data:{command: "switch",type: "property",id: $(this).data('id'), data:{currentStatus: $(this).hasClass('on')}},
                dataType: 'json',
            ...
            $('#deletenow').click(function(e){
              ...
              $.ajax({
                url:'action.php',
                method:'post',
                data:{command: "delete",type: "property",id: id},
                dataType: 'json',
                success:function(xhr){
                    console.log(xhr)
                  if(xhr.success == true)

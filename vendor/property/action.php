    ...
    if(isset($_POST['command']) && !empty($_POST['command']) ){
        ...
        if($type == 'property' || $type == 'vendor'){
            if($command == 'switch'){
                ...
                $sql = "UPDATE `".$type."` SET `status` = '".$status."' WHERE (`vendor_id` = '$vendor_id') AND `".$type."_id` = '".$id."'";
                $data = '{"currentStatus":'.$status.'}';
            }else if($command == 'delete'){
                $sql = "UPDATE `".$type."` SET `status` = '-1' WHERE (`vendor_id` = '$vendor_id') AND `".$type."_id` = '".$id."'";
                $data = '{}';
            }
                ...
                && ( ($_POST['ownership'] == 'Sale' && $_POST['duration'] == '') || ($_POST['ownership'] == 'Rental' && preg_match('/^[0-9]+/',$_POST['duration'])) )
                    ...
                    foreach($_FILES['pictures']['name'] as $i => $value){
                        ...
                        if(move_uploaded_file($_FILES['pictures']['tmp_name'][$i],$filename.'/'.time().'.'.$extension)){
                            array_push($pictures,'../vendor/storage/'.$vendor_id.'/'.time().'.'.$extension);
                        }else{
                    ...
                    $sql = "INSERT INTO `".$id."`(`vendor_id`,`amenities`,`facilities`,`pictures`,`type`,`bedroom`,`bathroom`,`price`,`rules`,`ownership`,`duration`,`guests`,`country`,`state`,`city`,`district`,`full_address`,`featured`,`views`,`status`,`date`) VALUES('$vendor_id','$amenities','$facilities','$pictures','$property','$bedroom','$bathroom','$price','$rules','$ownership','$duration','$guests','$country','$state','$city','$district','$full_address','0','0','0',NOW())";
                    $data = '{}';
                }else{

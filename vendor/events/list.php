<?php
  ...
  $sql = "SELECT * FROM `event` WHERE (`vendor_id` = '$vendor_id') AND `status` != '-1'";
...
<html lang="en">
  ...
  <body class="az-body az-body-sidebar">
    ...
    <?php
      $options = array('page' => 'events','subpage'=>'list');
    ...
    <div class="az-content-header d-block d-md-flex">
        <div>
          <h2 class="az-content-title tx-24 mg-b-5 mg-b-lg-8">Manage events</h2>
          <p class="mg-b-0">View, delete and modify the status of any event.</p>
        </div>
            ...
            <table id="eventList" class="table" style="width: 100%;">
              <thead>
                <tr role="row">
                  ...
                  <th rowspan="1" colspan="1" style="width: 10%;">Event ID</th>
                  <th rowspan="1" colspan="1" style="width: 25%;">Title</th>
                  <th rowspan="1" colspan="1" style="width: 25%;">Category</th>
...
EOT;
            for($i = 0; $i < count($result); $i++){
              ...
              echo <<<EOT
              <tr>
                <td class="details-control" data-event='{$event}' style="cursor:pointer;background:url('../../img/details_open.png') no-repeat center center;"></td>
                <td>{$result[$i]['event_id']}</td>
                <td>{$result[$i]['title']}</td>
                <td>{$category}</td>
                <td>{$result[$i]['price']}</td>
                <td style="display:flex;justify-content:space-around;align-items:flex-start">
                  <div class="az-toggle az-toggle-success switchstatus {$status}" data-id="{$result[$i]['event_id']}"><span></span></div>
                  <img src="../../img/cancel.svg" class="deleterow" data-toggle="modal" data-target="#deletemodal" style="cursor:pointer;" data-id="{$result[$i]['event_id']}" width="30px"/>
                </td>
            </tr>
EOT;
            }
        ...
        $(document).ready(function() {
            ...
            $('.switchstatus').click(function(e){
              ...
              $.ajax({
                url:'action.php',
                method:'post',
                data:{command: "switch",type: "event",id: $(this).data('id'), data:{currentStatus: $(this).hasClass('on')}},
                dataType: 'json',
            ...
            $('#deletenow').click(function(e){
              ...
              $.ajax({
                url:'action.php',
                method:'post',
                data:{command: "delete",type: "event",id: id},
                dataType: 'json',
                success:function(xhr){
                  if(xhr.success == true)

<?php
  ...
  $sql = "SELECT * FROM `insurance` WHERE (`vendor_id` = '$vendor_id') AND `status` != '-1'";
...
<html lang="en">
  ...
  <body class="az-body az-body-sidebar">
    ...
    <?php
      $options = array('page' => 'insurance','subpage'=>'list');
...
EOT;
            for($i = 0; $i < count($result); $i++){
              ...
              <tr>
                ...
                <td>{$result[$i]['insurance_id']}</td>
                <td>{$result[$i]['title']}</td>
                <td>{$result[$i]['category']}</td>
                <td>{$result[$i]['price']}</td>
                <td style="display:flex;justify-content:space-around;align-items:flex-start">
                  <div class="az-toggle az-toggle-success switchstatus {$status}" data-id="{$result[$i]['insurance_id']}"><span></span></div>
                  <img src="../../img/cancel.svg" class="deleterow" data-toggle="modal" data-target="#deletemodal" style="cursor:pointer;" data-id="{$result[$i]['insurance_id']}" width="30px"/>
                </td>
            </tr>
EOT;
            }
        ...
        $(document).ready(function() {
            ...
            $('.switchstatus').click(function(e){
              ...
              $.ajax({
                url:'action.php',
                method:'post',
                data:{command: "switch",type: "insurance",id: $(this).data('id'), data:{currentStatus: $(this).hasClass('on')}},
                dataType: 'json',
            ...
            $('#deletenow').click(function(e){
              ...
              $.ajax({
                url:'action.php',
                method:'post',
                data:{command: "delete",type: "insurance",id: id},
                dataType: 'json',
                success:function(xhr){
                    console.log(xhr)
                  if(xhr.success == true)

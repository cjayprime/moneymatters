<?php
    ...
    $item = isset($_GET['item']) ? mysqli_real_escape_string($database,$_GET['item']) : 'false';
    $category = isset($_GET['category']) ? mysqli_real_escape_string($database,$_GET['category']) : 'risk';
    ...
    //Motor insurance
    $vehicle_chasis = isset($_GET['vehicle_chasis']) ? mysqli_real_escape_string($database,$_GET['vehicle_chasis']) : '';
    $vehicle_engine = isset($_GET['vehicle_engine']) ? mysqli_real_escape_string($database,$_GET['vehicle_engine']) : '';
    $vehicle_type = isset($_GET['vehicle_type']) ? mysqli_real_escape_string($database,$_GET['vehicle_type']) : '';
    $vehicle_value = isset($_GET['vehicle_value']) ? mysqli_real_escape_string($database,$_GET['vehicle_value']) : '';
    $vehicle_registration = isset($_GET['vehicle_registration']) ? mysqli_real_escape_string($database,$_GET['vehicle_registration']) : '';
    ...
    $allVehicles = array($vehicle_chasis, $vehicle_engine, $vehicle_type, $vehicle_value, $vehicle_registration);
    $allColumns = array('description','tags');
    $addSql = '';
    for($j = 0; $j < count($allColumns); $j++){
      for($i = 0; $i < count($allVehicles); $i++){
        if(!empty($allVehicles[$i])){
          $addSql .= "(`".$allColumns[$j]."` LIKE '%".$allVehicles[$i]."%')";
          //Add `OR` if next item isn't empty
          if(isset($allVehicles[$i + 1]) && !empty($allVehicles[$i + 1])){
            $addSql .= ' OR ';
          }
        }
      }
    ...
    $sql = "SELECT * FROM `insurance` INNER JOIN `vendor` WHERE ".$addSql."
...
<html lang="en">
  ...
  <body>
    ...
    <?php
      $options = array('page' => 'insurance', 'subpage' => 'results');
